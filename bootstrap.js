...
//				console.log('profToolkit=',profToolkit);
				stackDOMJson = [
					{nodeToClone:'PUIsync', identifier:'[label="Create New Profile"]', label:'Create New Profile', class:'PanelUI-profilist create', hidden:null, id:null, oncommand:null, status:null, tooltiptext:null, signedin:null, defaultlabel:null, errorlabel:null,  addEventListener:['command',createUnnamedProfile,false], style:'-moz-appearance: none; padding: 10px 0px 10px 15px; height: 100%; flex-grow:1; background-color:blue;border:0;display:flex;'},
					{nodeToClone:'PUIsync', identifier:'[path="' + ini[profToolkit.selectedProfile.name].props.Path + '"]', label:profToolkit.selectedProfile.name, class:'PanelUI-profilist', hidden:null, id:null, oncommand:null, tooltiptext:null, signedin:null, defaultlabel:null, errorlabel:null, status:'active', addEventListener:['command', makeRename, false], style:'-moz-appearance: none; padding: 10px 0px 10px 15px; height: 100%; flex-grow:1; background-color:blue;border:0;display:flex;', props:{profpath:ini[profToolkit.selectedProfile.name].props.Path}}
				];
...
//					console.log('splicing p = ', ini[p], 'stackDOMjson=', stackDOMJson);
					stackUpdated = true;
					(function(pClosure) {
						var objToSplice = {nodeToClone:'PUIsync', identifier:'[path="' + ini[pClosure].props.Path + '"]', label:p, class:'PanelUI-profilist', hidden:null, id:null, oncommand:null, tooltiptext:null, signedin:null, defaultlabel:null, errorlabel:null,  status:'inactive', addEventListener:['command', launchProfile, false], addEventListener2:['mousedown', makeRename, false], style:'-moz-appearance: none; padding: 10px 0px 10px 15px; height: 100%; flex-grow:1; background-color:red;border:0;display:flex;', props:{profpath:ini[pClosure].props.Path}};
...
//		console.log('in json arr = ', i);
		...
		if (!el) {
			...
			//old full style::-moz-appearance:none; padding:10px 0 10px 15px; margin-bottom:-1px; border-top:1px solid rgba(24,25,26,0.14); border-bottom:1px solid transparent; border-right:0 none rgb(0,0,0); border-left:0 none rgb(0,0,0);
			var toolbarbuttonJSON = ['xul:box', {style:'display:flex; border-top-width: 1px; border-top-style: solid; border-top-color: rgba(24, 25, 26, 0.14);', class:'profilist-tbb-box'}, ['xul:toolbarbutton', {label:'newly created', class:'profilist-tbb newly-created', status:'active', style:'-moz-appearance: none; padding: 10px 0px 10px 15px; height: 100%; flex-grow:1; background-color:red;border:0;display:flex;'}]];
...
//			//console.log('PUIcs.style.width',PUIcs.style.width);
			...
			['xul:vbox', {id:'profilist_box'},
				['xul:stack', {key:'profilist_stack', style:'width:100%; overflow:hidden; transition:height 250ms;'},
					['xul:box', {style:'display:flex; border-top-width: 1px; border-top-style: solid; border-top-color: rgba(24, 25, 26, 0.14);', class:'PanelUI-profilist-box'}, ['xul:toolbarbutton', {'id':'profilistLoading', label:'Loading Profiles...', disabled:'true', class:'PanelUI-profilist profilistLoading', status:'active', style:'-moz-appearance:none; padding:10px 0 10px 15px; margin-bottom:-1px; border-top:1px solid rgba(24,25,26,0.14); border-bottom:1px solid transparent; border-right:0 none rgb(0,0,0); border-left:0 none rgb(0,0,0);', key:'profilistLoading'}]]
				]
			];
			var referenceNodes = {};
